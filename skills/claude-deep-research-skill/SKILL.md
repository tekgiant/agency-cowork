---
name: deep-research
description: Conduct enterprise-grade research using WorkIQ to search internal M365 data via Microsoft Graph. Use when user needs comprehensive analysis of enterprise data (emails, meetings, Teams chats, files, wikis), verified claims, or synthesis across internal sources. Also use for generating grounded executive status reports for the your program. Triggers include "deep research", "comprehensive analysis", "research report", "executive status report", "program status", "generate exec report", or any request requiring synthesis of internal enterprise data.
---
...
**Purpose:** Deliver citation-backed, verified research reports using WorkIQ to search enterprise M365 data via Microsoft Graph. Pipeline: Scope → Plan → Retrieve (WorkIQ) → Triangulate → Synthesize → Critique → Refine → Package. Includes source credibility scoring and progressive context management.
...
```
Request Analysis
├─ Simple lookup? → STOP: Use a single WorkIQ query, not this skill
├─ Debugging? → STOP: Use standard tools, not this skill
├─ program status report? → STOP: Use weekly-report skill instead (handles all exec reports)
│   └─ Includes: weekly status, exec reports, program reviews, PEC summaries, monthly readouts
└─ Complex enterprise analysis needed? → CONTINUE (enterprise data research via WorkIQ)

Mode Selection (Enterprise Research via WorkIQ)
...
- **Codename-Cr boundaries**: Distinguish between FACTS (from sources) and SYNTHESIS (your analysis)
...
**Phase 3 RETRIEVE - Mandatory Parallel WorkIQ Search:**
1. **Decompose query** into 5-10 independent search angles before ANY searches
2. **Launch ALL WorkIQ queries in single message** with multiple tool calls (NOT sequential)
...
```
[Single message with 8+ parallel WorkIQ calls]
WorkIQ #1: "What emails this week discuss <Topic> for <Program>?"
WorkIQ #2: "What meetings were held about <Program> <Topic>?"
WorkIQ #3: "What Teams chat messages mention <Program> milestones or risks?"
WorkIQ #4: "What files or documents were shared about <Program> <Topic>?"
WorkIQ #5: "What are the latest PEC meeting notes for <Program>?"
WorkIQ #6: "What executive review pre-reads discuss <Program>?"
Task agent #1: Deep-dive on specific risk area
Task agent #2: Cross-program dependency analysis
```

**❌ WRONG (sequential execution):**
```
WorkIQ #1 → wait for results → WorkIQ #2 → wait → WorkIQ #3...
```

**✅ RIGHT (parallel execution):**
```
All WorkIQ queries + agents launched simultaneously in one message
```
...
**File Organization (CRITICAL - Codename-Cn Accessibility):**

**1. program Executive Reports — Route to Program Subfolder:**
- **ALWAYS** save to: `memory/WeeklyReports/<Program>/Week of MM-DD-YYYY.md`
- Use Monday's date for the week identifier
- Program folders:
  - `memory/WeeklyReports/program 200/` — Gen2 "Codename-A" reports
  - `memory/WeeklyReports/program 300/` — Gen3 "Codename-B" reports
  - `memory/WeeklyReports/program 400/` — Gen4 "Codename-C" reports
- Examples:
  - program 300 weekly → `memory/WeeklyReports/program 300/Week of 03-03-2026.md`
  - program 200 exec review → `memory/WeeklyReports/program 200/Week of 03-03-2026.md`
  - program 400 status → `memory/WeeklyReports/program 400/Week of 03-03-2026.md`

**2. General Enterprise Research — Create Organized Folder:**
- For non-program or general enterprise research: `~/Documents/[TopicName]_Research_[YYYYMMDD]/`
- Extract Codename-Cn topic name from research question (remove special chars, use underscores/CamelCase)
...
- If final sections: Generate complete bibliography, verify report, Codename-Cnup state file
...
- Delete continuation_state.json (Codename-Cnup)
...
- **Directness**: State findings Codename-Crly without embellishment
...
- Codename-Cr facts vs. analysis distinction
...
**Use when:**
- Comprehensive enterprise data analysis (10+ internal sources needed)
- Synthesizing across emails, meetings, Teams chats, files, and wikis via WorkIQ
- Cross-referencing internal documents and communications
- Technical trade-off analysis requiring deep investigation
- Any request requiring deep synthesis of Microsoft 365 enterprise data that is NOT a status report

**Do NOT use:**
- Simple lookups (use a single WorkIQ query directly)
- Debugging (use standard tools)
- Public web research (this skill uses WorkIQ for enterprise data, not web search)
- 1-2 query answers
- Time-sensitive quick answers
- **Any program status reports** (use weekly-report skill — handles exec reports, weekly updates, program reviews, PEC summaries)
- **Combined multi-program reports** (weekly-report skill runs once per program)
...
## program Executive Report Mode — Deferred to weekly-report Skill

**All program status report requests are handled by the `weekly-report` skill.**

This includes: weekly status updates, executive reports, program reviews, PEC summaries, monthly readouts, and any request requiring synthesis of internal your program data.

**If this skill is invoked for a status report, immediately defer:**
1. STOP execution of this skill
2. Read `skills/weekly-report/skills/weekly-report/SKILL.md`
3. Execute the weekly-report skill's 7-phase workflow instead

See: `skills/weekly-report/skills/weekly-report/SKILL.md` for the full program Executive Report Mode pipeline (WorkIQ search, source evaluation, fact verification, style conformance, and output routing to `memory/WeeklyReports/<Program>/`).
